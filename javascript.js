...
function sortBooks() {
  ...
  let arrayChanged = preSortedLibrary.find(
    (obj) => obj !== myLibrary[preSortedLibrary.indexOf(obj)]
  );
...
function initializeForm() {
  ...
  form.addEventListener("submit", (event) => {
    // Listener to add book that is submitted via form
...
function addTestBooks() {
  ...
  addBookToLibrary(
    new Book({ title: "Another Sample Book", author: "A. U. Thor", pages: 510 })
  );
}
